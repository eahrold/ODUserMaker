...
@implementation NSTextField (blankIsNil)
-(NSString*)blankIsNil{
...
@implementation NSString (blankIsNil)
-(NSString*)blankIsNil{
...
@implementation NSArray (requiredFields)
-(BOOL)requiredFields {
    for(NSTextField* field in self){
        if(field.isBlank)return NO;
    }
    return YES;
}
@end

@implementation NSArray (requiredStrings)
-(BOOL)requiredStrings {
    for(NSString* string in self){
        if(string.isBlank)return NO;
    }
    return YES;
}
@end
